...
test('oauth getTokens', assert => {
    ...
    nock('https://api.patreon.com').post('/oauth2/token').reply(200, function(uri, body) {
        const params =
    ...
    nock('https://api.patreon.com').post('/oauth2/token').replyWithError('Oh geeze')
    ...
    nock('https://api.patreon.com').post('/oauth2/token').reply(200, () => {
        return mockInvalidGrant
    })
    ...
    getTokens('code', '/redirect')
        ...
        .catch(err => {
            ...
            assert.equal(
                ...
                'err message should include grant type'
            )
        })
})
